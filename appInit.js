...
async function initializer() {
	try {
		...
		await connection.query(`CREATE DATABASE IF NOT EXISTS RuralWaterSupplyAndSanitationSystem`, function (err, result) {
				if (err) throw err;
		});

		await connection.query(`use RuralWaterSupplyAndSanitationSystem`, function (err, result) {
			if (err) throw err;
		});
		...
		await connection.query(`CREATE TABLE IF NOT EXISTS Jobs 
			(
		...
		await connection.query(`CREATE TABLE IF NOT EXISTS WaterSources
			(
		...
		await connection.query(`CREATE TABLE IF NOT EXISTS WaterUsages
			(
				WSID int not null,
		...
		await connection.query(`create table if not exists SanitationSystems 
		(
		...
		await connection.query(`create table if not exists Families 
		(
		...
		await connection.query(`create table if not exists Donations 
		(
		...
		await connection.query(`create table if not exists Expenditures 
		(
		...
		await connection.query(`select * from Jobs where Designation="Admin"`, function (err, result) {
			if(err) throw err;
			if(result.length === 0) {
				connection.query(`INSERT into Jobs(Designation,Shift) values('Admin', 'Full-time')`, function (err, result) {
					if(err) throw err;
				});
			}
		});

		await connection.query(`select * from Locations where Pincode=000000`, function (err, result) {
			if(err) throw err;
			if(result.length === 0) {
				connection.query(`INSERT into Locations(Pincode, Panchayat, District) values(000000, 'Panchayat', 'District')`, function (err, result) {
					if(err) throw err;
				});
			}
		});
		...
		for (let admin of admins) {
			connection.query(`select * from Employees where Username="${admin.Username}"`,function (err, result) {
				if(err) throw err;
				if(result.length === 0) {
					connection.query(`INSERT into Employees(FName,LName,EContact,JobCode,Pincode,Username,Password) values('${admin.FName}', '${admin.LName}', ${admin.EContact}, ${admin.JobCode}, ${admin.Pincode}, '${admin.Username}', '${bcrypt.hashSync(admin.Password, +config.hashsecret)}')`, 
						function (err, result) {
							if(err) throw err;
